use diesel::{
    query_dsl::methods::{ OrderDsl, SelectDsl },
    r2d2::{ ConnectionManager, Pool },
    Connection,
    ExpressionMethods,
    MysqlConnection,
    RunQueryDsl,
    SelectableHelper,
};
use thiserror::Error;
use crate::db::{
    connection_pool,
    models::{ CreateTodo, TodoModel },
    schema,
    DbConnectionPoolError,
};

#[derive(Error, Debug)]
pub enum Error {
    #[error("Connection pool init error: {0} ")] ConnectionPool(#[from] DbConnectionPoolError),
    #[error("R2D2 DB pool build error: {0}")] R2D2(#[from] r2d2::Error),
    #[error("DB error: {0}")] Diesel(#[from] diesel::result::Error),
}
pub struct Service {
    conn_pool: Pool<ConnectionManager<MysqlConnection>>,
}

impl Service {
    pub fn new() -> Result<Self, Error> {
        Ok(Self {
            conn_pool: connection_pool()?,
        })
    }

    pub fn create(&self, request: CreateTodo) -> Result<TodoModel, Error> {
        let mut conn = self.conn_pool.get()?;
        let result = conn.transaction(|conn| {
            diesel::insert_into(schema::todos::table).values(&request).execute(conn)?;

            schema::todos::table
                .order(schema::todos::id.desc())
                .select(TodoModel::as_select())
                .first(conn)
        })?;

        Ok(result)
    }
}
